...
def buttonClick(number):
    global operator
    operator = operator + str(number)  # Concatenate the clicked number to the operator string
    input_value.set(operator)  # Update the input field with the new operator string

def buttonClear():
    global operator
    operator = ""  # Clear the operator string
    input_value.set("")  # Clear the input field

def buttonEqual():
    global operator
    result = str(eval(operator))  # Evaluate the expression and convert result to string
    input_value.set(result)  # Update the input field with the result
    operator = ""  # Clear the operator string for next calculation

# Main application window
main = Tk()
main.title("Calculator")

# StringVar to store the input value
operator = ""
input_value = StringVar()

# Entry widget for displaying the calculator input
display_text = Entry(main, font=("arial", 20, "bold"), textvariable=input_value, bd=30, insertwidth=4,
                     bg="powder blue", justify=RIGHT)
display_text.grid(columnspan=4)  # Span across 4 columns

# Define and place all calculator buttons using grid layout
...
main.mainloop()  # Start the GUI event loop
